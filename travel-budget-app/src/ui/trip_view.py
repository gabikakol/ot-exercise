...
class TripView:
    ...
    def start(self):
        ...
        header_label = ttk.Label(master=self._window, text=f"Expenses of the {self.trip_name} trip:")
        ...
        stats_button = ttk.Button(master=self._window, text="Trip statistics", command = self.trip_stats_handle)
    ...
    def init_expense(self, exp):
        if exp.trip_id == self.trip_id:
            exp_label = ttk.Label(master=self._window, text=f"{exp.description}: €{exp.amount}, category: {exp.category}")
