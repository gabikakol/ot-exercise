...
class TripStats:
    ...
    def start(self):
        ...
        stats_label = ttk.Label(master=self._window, text=f"Statistics of the {self.trip_name} trip:")
        ...
        back_button = ttk.Button(master=self._window, text="Back", command=self.trip_view_handle)
    ...
    def spent_total(self):
        ...
        spent_total_label = ttk.Label(master=self._window, text=f"Spent in total: €{sum}")
        spent_total_label.grid(padx=5,pady=5)
        return sum

    def trip_duration(self):
        duration_label = ttk.Label(master=self._window, text=f"Duration of the trip: {self.current_trip.duration} days")
        duration_label.grid(padx=5,pady=5)

    def spent_per_day(self, sum):
        average = sum/int(self.current_trip.duration)
        average_day_label = ttk.Label(master=self._window, text=f"Spent per day on average: €{average}")
